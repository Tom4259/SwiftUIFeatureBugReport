...
public class GitHubService {
    ...
    public func loadIssues(type: IssueType = .all) async {
        ...
        do {
            ...
            case .all:
                // For "all", we want issues with either label, not both
                // GitHub API doesn't support OR logic in labels, so we'll filter after fetching
                break // Don't add any label filter - get all issues
            }
            ...
            switch type {
            case .all:
                // Show issues that have either bug OR feature-request labels (or both)
                filteredIssues = fetchedIssues.filter { issue in
                    issue.isBug || issue.isFeatureRequest
                }
            case .bugs, .features:
                // For specific types, GitHub already filtered for us
